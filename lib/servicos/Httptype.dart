...
abstract  class   Httptype {
  ...
  // metódos usado na classe CadastroP
  static Future<Map> postCP(dynamic n,dynamic cpf,dynamic c) async{
    http.Response reposta =  await http.post(cadastrop,
        body: 
        {
          "nome" : n.text,
          "cpf" : _ajeitacpf(cpf.text),
          "carteiraSUS": c.text  
        },
        headers: 
        {
  ...
  static Future<bool> deleteP(String valor) async{
    if(valor.length == 11){
      http.Response resposta =  await http.delete(deletep + "cpf=" + valor);   
       return _auxiliardp(resposta);
    }else{
      http.Response resposta = await http.delete(deletep+ "carteiraSUS=" + valor);
      return _auxiliardp(resposta);
    }
  ...
  //metódos auxilares para economia de código ou reajuste de valor da entrada pra ser enviado
   static bool _auxiliardp(http.Response r){
     if(r.statusCode == 200){
       return true;
     }else{
       return false;
   ...
   static String _ajeitacpf(String cpf){
